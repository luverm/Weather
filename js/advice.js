...
export function advise(weather) {
  ...
  if (cond === "storm") return "⛈️ Thunderstorms — stay indoors and unplug sensitive gear.";
  if (cond === "snow") {
  ...
  if (nextRain) {
    ...
    if (mins > 0 && mins < 180) return `🌧️ Rain likely in ~${mins >= 60 ? Math.round(mins / 60) + "h" : mins + "m"} — bring a light shell.`;
  }
  if (gust > 45) return "💨 Blustery — anything loose will sail away.";
  if (t <= -10) return "🥶 Frostbite weather — cover exposed skin.";
  if (t <= 0) return "❄️ Freezing — heavy coat, scarf, gloves.";
  ...
  if (t <= 32) {
    if (uv >= 7) return "☀️ Hot with strong sun — hydrate and stay shaded.";
