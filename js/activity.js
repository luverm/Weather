...
function reasonsFor(window, hours, unit) {
  if (!window) return [];
  ...
  const reasons = [];
  const fmtT = (c) => Math.round(unit === "F" ? c * 9 / 5 + 32 : c);
  if (avgT != null) reasons.push(`${fmtT(avgT)}° feel`);
...
export function findActivityWindows(weather, { unit } = {}) {
