...
export function buildInsights(weather, { fmtTime, weekday, convertTemp } = {}) {
  const out = [];
  ...
  if (coldest && warmest && warmest.t - coldest.t >= 4) {
    out.push({
      ...
      value: `${Math.round(toDisp(coldest.t))}° at ${fmt(coldest.ts)}`,
      ts: coldest.ts,
    });
    out.push({
      icon: ICONS.warm, label: "Warmest",
      value: `${Math.round(toDisp(warmest.t))}° at ${fmt(warmest.ts)}`,
      ts: warmest.ts,
    });
  }
  ...
  if (days.length >= 2) {
    ...
    if (hotDay && coolDay && hotDay !== coolDay) {
      out.push({
        ...
        value: `${Math.round(toDisp(hotDay.tempMax))}° on ${dow(hotDay.time)}`,
