...
export function buildInsights(weather, { fmtTime, weekday, fmtWind, fmtTemp } = {}) {
  const out = [];
  ...
  if (peakGust && peakGust.v >= 20) {
    out.push({
      ...
      value: `${wind(peakGust.v)} at ${fmt(peakGust.ts)}`,
      ts: peakGust.ts,
    });
  }
  ...
  if (coldest && warmest && warmest.t - coldest.t >= 4) {
    out.push({
      ...
      value: `${temp(coldest.t)} at ${fmt(coldest.ts)}`,
      ts: coldest.ts,
    });
    out.push({
      icon: ICONS.warm, label: "Warmest",
      value: `${temp(warmest.t)} at ${fmt(warmest.ts)}`,
      ts: warmest.ts,
    });
  }
  ...
  if (days.length >= 2) {
    ...
    if (hotDay && coolDay && hotDay !== coolDay) {
      out.push({
        ...
        value: `${temp(hotDay.tempMax)} on ${dow(hotDay.time)}`,
