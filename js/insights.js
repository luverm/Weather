...
export function buildInsights(weather, { fmtTime, weekday, fmtTemp, fmtWind } = {}) {
  const out = [];
  ...
  const fmtW = fmtWind || ((kmh) => `${Math.round(kmh)} km/h`);
  ...
  if (peakGust && peakGust.v >= 20) {
    out.push({
      ...
      value: `${fmtW(peakGust.v)} at ${fmt(peakGust.ts)}`,
      ts: peakGust.ts,
    });
  }
  ...
  if (coldest && warmest && warmest.t - coldest.t >= 4) {
    out.push({
      ...
      value: `${fmtT(coldest.t)} at ${fmt(coldest.ts)}`,
      ts: coldest.ts,
    });
    out.push({
      icon: ICONS.warm, label: "Warmest",
      value: `${fmtT(warmest.t)} at ${fmt(warmest.ts)}`,
      ts: warmest.ts,
    });
  }
  ...
  if (days.length >= 2) {
    ...
    if (hotDay && coolDay && hotDay !== coolDay) {
      out.push({
        ...
        value: `${fmtT(hotDay.tempMax)} on ${dow(hotDay.time)}`,
