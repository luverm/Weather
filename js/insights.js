...
export function buildInsights(weather, { fmtTime, weekday, convertTemp } = {}) {
  ...
  const t2 = convertTemp || ((c) => c);
  const tDeg = (c) => `${Math.round(t2(c))}°`;
  ...
  if (coldest && warmest && warmest.t - coldest.t >= 4) {
    out.push({
      ...
      value: `${tDeg(coldest.t)} at ${fmt(coldest.ts)}`,
      ts: coldest.ts,
    });
    out.push({
      icon: ICONS.warm, label: "Warmest",
      value: `${tDeg(warmest.t)} at ${fmt(warmest.ts)}`,
      ts: warmest.ts,
    });
  }
  ...
  if (days.length >= 2) {
    ...
    if (hotDay && coolDay && hotDay !== coolDay) {
      out.push({
        ...
        value: `${tDeg(hotDay.tempMax)} on ${dow(hotDay.time)}`,
