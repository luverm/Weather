  ...
  if (d.hourly?.time) {
    for (let i = 0; i < d.hourly.time.length && hourly.length < 24; i++) {
      ...
      hourly.push({
        ...
        clouds: d.hourly.cloud_cover?.[i] ?? null,
  ...
  if (daily.time) {
    for (let i = 0; i < daily.time.length; i++) {
      ...
      dailyForecast.push({
        ...
        windDirDom: daily.wind_direction_10m_dominant?.[i] ?? null,
...
function mock(lat, lon) {
  ...
  return {
    ...
    hourly: Array.from({ length: 24 }, (_, i) => ({
      ...
      humidity: 60 + Math.round(Math.sin(i / 3) * 12),
      clouds: 35 + Math.round(Math.sin(i / 4 + 1) * 30),
